...
ggplot()+
  ...
  geom_point(data=mydata,aes(x=Latitude,y=Longitude,size=Number),
...
ggsave("Fig. 1a_time.pdf",width=4,height=4)  
...
ggplot()+
  ...
  geom_point(data=mydata,aes(x=Latitude,y=Longitude),fill="#6495ED",alpha=0.8,shape=21,size=2)
ggsave("Fig.1a.pdf",width=2.9,height=2.9)
...
ggsave("Fig.1c.pdf",width=2.7,height=3.6)
...
ggplot(data, aes(x=quality, y=hallmark)) +  
  ...
  ylim(0,230)+
  theme(legend.position = "none")
ggsave("Fig.1c1.pdf",width=2.7,height=3.6)

#host contamination
ggplot(data, aes(x=quality, y=contamination)) +  stat_boxplot(geom = 'errorbar',width=0.2,linetype=1,size=0.3)+
  geom_boxplot(width=0.45,outlier.color="white",notch=F,size = 0.3,fill="lightgray")+
  geom_jitter(color="black",width=0.2,alpha=0.2,size=1.1)+
  theme(axis.text.x=element_text(angle=0,hjust=0.2, vjust=0.5)) +
  ...
  theme(axis.title.x = element_text(size=16),axis.title.y = element_text(size=16),
        ...
        axis.line = element_line(colour = "black", size = 0.5))+
  theme(legend.position = "none")
ggsave("Fig.1c2.pdf",width=2.6,height=3.6)