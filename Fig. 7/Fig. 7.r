...
mantel.partial(OTUs,env,geo,permutations = 999,method="pearson",na.rm=T)
...
qcorrplot(rcorr(env), type = "upper", diag = F, grid_size = 0.4,grid_col = "lightgray") +
  ...
  set_corrplot_style(colours  =c("#7AC5CD","white","#6BB5FF")) +
...
data$class=factor(data$class, levels=c("Nucleocytoviricota","Uroviricota"))
...
ggplot(data,aes(x=class,y=pairwise_NST))+
  ...
  theme(panel.grid.major=element_blank(),
        ...
        axis.text.x = element_text(hjust = 0.5,size=14,angle = 0,colour = 'black'),
...
ggplot(data,aes(x=class,y=Ratio,fill=level))+
  geom_bar(stat="identity", color="black",width=0.55,size=0.2,position="stack", aes(fill=level),alpha=0.8)+
...
comm <- t(otu)
tree <- read.tree("Terl_trimal.faa.contree")
...
comm <- t(otu)
tree <- read.tree("NCLDV_trimal.faa.contree")
tax <- read.delim('taxanomy.txt', row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE)
save.wd=("Fig. 7/iCAMP")
