...
data$Host_tax <- factor(data$Host_tax,levels=c('Planctomycetota', 'Nitrospirota','Chloroflexota','Thermotogota','Patescibacteria',
                                               'Actinobacteriota','Cyanobacteria','Firmicutes','Bacteroidota','Proteobacteria'))
...
data <- data.frame(
  sales,share,names)
ggplot()+
  geom_arc_bar(data=data,aes(x0 = 0, y0 = 0, r0 = 0, r = 1,amount=sales,explode=c(0,0),fill=names),stat="pie",alpha=0.9)+
...
ggplot(data, aes(x=Host_tax, y=PD))+
  ...
  theme(legend.position = "none")+theme(axis.ticks.y.left = element_blank())
